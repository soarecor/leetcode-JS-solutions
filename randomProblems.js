...
// if any two numbers can be multiplied so that the answer is greater than double the sum of all the elements 
// in the array. If not, return the string false. For example: if arr is [2, 5, 6, -6, 16, 2, 3, 6, 5, 3] then the 
// sum of all these elements is 42 and doubling it is 84. There are two elements in the array, 16 * 6 = 96 and 96 
// is greater than 84, so your program should return the string true.
...
// ************************************* NEW PROBLEM *************************************************
// Have the function StringExpression(str) read the str parameter being passed which will contain the written out 
// version of the numbers 0-9 and the words "minus" or "plus" and convert the expression into an actual final number 
// written out as well. For example: if str is "foursixminustwotwoplusonezero" then this converts to "46 - 22 + 10" 
//which evaluates to 34 and your program should return the final string threefour. If your final answer is negative 
...
// ************************************* NEW PROBLEM *************************************************
 ...
 // Algorithm
 // compare digit n and n + 1, where n is initially the second last digit in the number. If n < n+1, move n to 
 // to the end of the string.
 // Keep doing this until you reach thefirst digit in the number
...
const permutationStep = num => {
    ...
    for(let i = str.length - 2; i > -1 ; i--) {
        if(str[i] < str[i+1]) {
            return `${str.slice(0,i)}${str.slice(i+1)}${str[i]}`
        }  
    }
...
console.log(permutationStep(123410))